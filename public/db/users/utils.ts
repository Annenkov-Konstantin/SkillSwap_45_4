...
const maleNames = [
  "Александр", "Михаил", "Дмитрий", "Иван", "Алексей", "Сергей", 
  "Андрей", "Максим", "Владимир", "Роман", "Николай", "Павел",
  "Константин", "Артем", "Евгений", "Василий", "Георгий", "Тимур",
  "Даниил", "Кирилл", "Виктор", "Олег", "Станислав", "Федор"
];

const femaleNames = [
  "Екатерина", "Анна", "Ольга", "Мария", "София", "Елена", "Наталья",
  "Ирина", "Татьяна", "Юлия", "Анастасия", "Виктория", "Евгения",
  "Ксения", "Алиса", "Дарья", "Валерия", "Полина", "Вероника", "Марина",
  "Светлана", "Людмила", "Галина", "Лариса"
];

const lastNames = [
  "Иванов", "Смирнов", "Петров", "Соколов", "Кузнецов", "Попов",
  "Лебедев", "Козлов", "Новиков", "Морозов", "Волков", "Соловьев",
  "Васильев", "Зайцев", "Павлов", "Семенов", "Голубев", "Виноградов",
  "Богданов", "Воробьев", "Федоров", "Михайлов", "Беляев", "Тарасов"
];

const femaleLastNames = [
  "Иванова", "Смирнова", "Петрова", "Соколова", "Кузнецова", "Попова",
  "Лебедева", "Козлова", "Новикова", "Морозова", "Волкова", "Соловьева",
  "Васильева", "Зайцева", "Павлова", "Семенова", "Голубева", "Виноградова",
  "Богданова", "Воробьева", "Федорова", "Михайлова", "Беляева", "Тарасова"
];

const cities = [
  "Москва", "Санкт-Петербург", "Новосибирск", "Екатеринбург", "Казань",
  "Краснодар", "Владивосток", "Ростов-на-Дону", "Уфа", "Нижний Новгород",
  "Красноярск", "Пермь", "Воронеж", "Волгоград", "Омск", "Самара",
  "Челябинск", "Иркутск", "Тюмень", "Барнаул", "Сочи", "Калининград",
  "Ярославль", "Томск", "Хабаровск", "Владимир", "Ставрополь", "Тверь"
];
...
const generateEmailWithTranslit = (firstName: string, lastName: string, index: number): string => {
  const translitFirst = transliterateToLatin(firstName);
  const translitLast = transliterateToLatin(lastName);
  ...
  const formats = [
    `${translitFirst}.${translitLast}@${randomDomain}`,
    `${translitFirst[0]}.${translitLast}@${randomDomain}`,
    `${translitFirst}.${translitLast}${index}@${randomDomain}`,
    `${translitFirst}_${translitLast}@${randomDomain}`
  ];
  
  return formats[Math.floor(Math.random() * formats.length)];
};

// Функция для удаления дубликатов из массива (без Set)
...
const generateUniqueSkills = (count: number): string[] => {
  const skills: string[] = [];
  const maxSkills = 40; // У нас есть навыки с skill_001 по skill_040
...
export const generateSingleUser = (index: number): TUser => {
  ...
  const email = generateEmailWithTranslit(firstName, lastName, index);
  ...
  return {
    ...
    email,
    aboutMe: aboutMeTemplates[Math.floor(Math.random() * aboutMeTemplates.length)],
...
export const mergeUsers = (existingUsers: TUser[], newUsers: TUser[]): TUser[] => {
...
export const addUsersToExisting = (existingUsers: TUser[], count: number): TUser[] => {
  // Находим максимальный ID в существующих пользователях
  let maxExistingId = 0;
  for (const user of existingUsers) {
    const idNum = parseInt(user._id.split('_')[1]);
