...
export const HomeCatalog: FC = () => {
  ...
  const userListRequestStatus = useAppSelector(
    userListSelectors.selectUserListStatus
  );
  const userList = useAppSelector(userListSelectors.selectUserList);
  const userSkillListRequestStatus = useAppSelector(
    userSkillListSelectors.selectUserSkillListStatus
  );
  const isLoading =
    ...
    <div className={styles.container}>
      ...
      {isLoading ? (
        ...
        <div className={styles.main_content}>
          {/*Пример отображения секции с карточками "Популярное"*/}
          <PopularSwapList />
          {/*        { cards.map( card =>
        (<UserCard
          user={card.user}
