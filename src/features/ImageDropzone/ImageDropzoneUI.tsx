...
export const ImageDropzone: FC<TImageDropzoneProps> = ({
  handleDelete,
  onImagesAdded,
  onImageRemoved,
  images = []
  ...
  const handleRemoveFile = (index: number, file: any) => {
    handleDelete?.(file);
    onImageRemoved?.(index, file);
  };

  const thumbs = images.map((file: any, index: number) => (
    <div className={styles.thumb} key={`${file.name || file}-${index}`} onClick={(e) => e.stopPropagation()}>
...
//   // ВАЖНО: добавляем деструктуризацию
//   const { getRootProps, getInputProps } = useDropzone({
//     accept: {
//       'image/*': []
//     },
...
//       // Вызываем колбэк с новыми файлами
//       onImagesAdded?.(acceptedFiles);
//     }
//   });

//   const handleRemoveFile = (index: number, targetFile: File & { preview: string }) => {
//     setFiles((prevFiles) => prevFiles.filter((_, i) => i !== index));
//     URL.revokeObjectURL(targetFile.preview);
//     handleDelete?.(targetFile);
//     onImageRemoved?.(index, targetFile);
//   };

//   const thumbs = files.map((file, index) => (
//     <div className={styles.thumb} key={`${file.name}-${index}`} onClick={(e) => e.stopPropagation()}>
//       <div className={styles.thumbInner}>
//         <img
//           src={file.preview}
//           className={styles.img}
